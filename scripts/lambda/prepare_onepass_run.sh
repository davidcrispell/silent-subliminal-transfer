...
ONEPASS_CONFIG="${2:-configs/wolf_sl_9b_eb16_onepass.yaml}"
...
silent-transfer validate "$ONEPASS_CONFIG" --repo-root "$REPO_ROOT"
python scripts/verify_onepass_followup.py \
  "$ONEPASS_CONFIG" --repo-root "$REPO_ROOT"
python scripts/reuse_run_data.py \
  "$SOURCE_CONFIG" \
  "$ONEPASS_CONFIG" \
  --repo-root "$REPO_ROOT"
python scripts/verify_onepass_followup.py \
  "$ONEPASS_CONFIG" --repo-root "$REPO_ROOT" --require-data
