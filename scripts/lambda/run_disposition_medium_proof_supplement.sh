...
CONFIG="${1:?usage: run_disposition_medium_proof_supplement.sh CONFIG [REPO_ROOT]}"
REPO_ROOT="${2:-$(pwd)}"
...
"$PYTHON" scripts/supplement_proof_paraphrases.py "$CONFIG" --repo-root "$REPO_ROOT"
...
required = {
    "original_raw_treatment": root / "data" / "raw_treatment.jsonl",
    "supplement_raw_treatment": root / "data" / "raw_treatment.supplement.jsonl",
    "augmented_raw_treatment": root / "data" / "raw_treatment.augmented.jsonl",
    "augmented_raw_manifest": root / "data" / "raw_treatment.augmented.manifest.json",
...
augmented = json.loads(required["augmented_raw_manifest"].read_text())
extra = augmented.get("extra", {})
if int(extra.get("original_prompt_count", -1)) != 10240:
    raise SystemExit("supplement manifest does not bind the original 10240-row bank")
if int(extra.get("supplement_prompt_count", -1)) != 512:
    raise SystemExit("supplement manifest does not bind exactly 512 added candidates")
payload = {
    "schema_version": 1,
    "status": "complete",
    ...
    "proof_supplement": {
        ...
        for key in (
            ...
            "supplement_prompt_count",
            "augmented_prompt_count",
            "valid",
            "valid_margin",
        )
    },
