...
BATCH_CONFIG="${2:-configs/wolf_sl_9b_batch500.yaml}"
...
silent-transfer validate "$BATCH_CONFIG" --repo-root "$REPO_ROOT"
python scripts/verify_large_batch_followup.py \
  "$BATCH_CONFIG" --repo-root "$REPO_ROOT"
python scripts/reuse_run_data.py \
  "$SOURCE_CONFIG" \
  "$BATCH_CONFIG" \
  --repo-root "$REPO_ROOT"
python scripts/verify_large_batch_followup.py \
  "$BATCH_CONFIG" --repo-root "$REPO_ROOT" --require-data
